    ...
    -> Point
{
  ...
  if (point1.is3D() && point2.is3D()) {
    double coordZ = (CGAL::to_double(point1.z()) * (1 - parameter)) +
                    (CGAL::to_double(point2.z()) * parameter);
    if (point1.isMeasured() && point2.isMeasured()) {
      double measure =
          (point1.m() * (1 - parameter)) + (point2.m() * parameter);
      return {coordX, coordY, coordZ, measure};
    }
    return {coordX, coordY, coordZ};
  }
  if (point1.isMeasured() && point2.isMeasured()) {
    double measure = (point1.m() * (1 - parameter)) + (point2.m() * parameter);
...
BezierCurve::derivative(double parameter, unsigned int order) const -> Point
{
  ...
  if (degree() < order) {
    // Return zero vector for derivatives beyond curve degree
    if (is3D()) {
      return {0, 0, 0};
    }
    return {0, 0};
  ...
  // Construct derivative curve control points
  ...
  for (size_t index = 0; index < _controlPoints.size() - 1; ++index) {
    ...
    if (is3D()) {
      double deltaZ = CGAL::to_double(_controlPoints[index + 1].z() -
                                      _controlPoints[index].z()) *
                      factor;
      if (isMeasured()) {
        double deltaM =
            (_controlPoints[index + 1].m() - _controlPoints[index].m()) *
            factor;
        derivativePoints.emplace_back(deltaX, deltaY, deltaZ, deltaM);
      } else {
      ...
      derivativePoints.emplace_back(Kernel::FT(deltaX), Kernel::FT(deltaY),
                                    Kernel::FT(NaN()), deltaM);
    } else {
