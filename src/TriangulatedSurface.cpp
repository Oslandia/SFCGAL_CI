...
public:
  ...
  operator()(HDS &hds) override
  {
    ...
    for (size_t i = 0; i < nrPatchs; i++) {
      ...
      CGAL::Point_3<K> const    point_c(tri[2]);

      if (edges.find(std::make_pair(pa, pb)) != edges.end() ||
          edges.find(std::make_pair(pb, point_c)) != edges.end() ||
          edges.find(std::make_pair(point_c, pa)) != edges.end()) {
      ...
      builder.add_vertex_to_facet(points[pa]);
      builder.add_vertex_to_facet(points[pb]);
      builder.add_vertex_to_facet(points[point_c]);
      edges.insert(std::make_pair(pa, pb));
      edges.insert(std::make_pair(pb, point_c));
      edges.insert(std::make_pair(point_c, pa));
      builder.end_facet();
    }
