    ...
    -> std::unique_ptr<Geometry>
{
  ...
  case TYPE_LINESTRING: {
    // Extract segments for interpolation
    SegmentStore store;
    extractSegments(geometry, store);

    return simplifyLineString(static_cast<const LineString &>(geometry),
                              squaredThreshold, preserveTopology, store);
  }

  case TYPE_MULTILINESTRING:
    return simplifyMultiLineString(
        static_cast<const MultiLineString &>(geometry), squaredThreshold,
        preserveTopology);

  case TYPE_POLYGON:
    return simplifyPolygon(static_cast<const Polygon &>(geometry),
                           squaredThreshold, preserveTopology);

  case TYPE_MULTIPOLYGON:
    return simplifyMultiPolygon(static_cast<const MultiPolygon &>(geometry),
                                squaredThreshold, preserveTopology);

  case TYPE_POLYHEDRALSURFACE:
    return simplifyPolyhedralSurface(
        static_cast<const PolyhedralSurface &>(geometry), squaredThreshold,
        preserveTopology);

  case TYPE_GEOMETRYCOLLECTION:
    return simplifyGeometryCollection(
