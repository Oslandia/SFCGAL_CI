...
auto
insertPointsFrom(const Geometry &baseGeometry, const Geometry &sourceGeometry,
                 double tolerance) -> std::unique_ptr<Geometry>
{
  ...
  case TYPE_POLYGON: {
    ...
    auto exteriorRing = insertPointsIntoLineString(basePolygon.exteriorRing(),
                                                   sourcePoints, tolerance);
    resultPolygon->setExteriorRing(std::move(exteriorRing));

    // Process interior rings
    for (size_t i = 0; i < basePolygon.numInteriorRings(); ++i) {
      const auto &hole = basePolygon.interiorRingN(i);
      auto newHole = insertPointsIntoLineString(hole, sourcePoints, tolerance);
      resultPolygon->addInteriorRing(std::move(newHole));
    }

    return std::unique_ptr<Geometry>(resultPolygon.release());
  }
  ...
  case TYPE_MULTIPOLYGON: {
    ...
    for (size_t i = 0; i < baseMultiPolygon.numGeometries(); ++i) {
      const auto &polygon = baseMultiPolygon.geometryN(i).as<Polygon>();
      ...
      // Process interior rings
      for (size_t j = 0; j < polygon.numInteriorRings(); ++j) {
        const auto &hole = polygon.interiorRingN(j);
        auto        newHole =
            insertPointsIntoLineString(hole, sourcePoints, tolerance);
        resultPolygon->addInteriorRing(std::move(newHole));
      }

      resultMultiPolygon->addGeometry(std::move(resultPolygon));
    }

    return std::unique_ptr<Geometry>(resultMultiPolygon.release());
  }
