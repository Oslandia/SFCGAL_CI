...
insertPolygonBoundaryIntoArrangement(Arrangement &arr, const Polygon &polygon)
{
  ...
  const size_t      numSegments =
      exterior.isClosed() ? exterior.numPoints() - 1 : exterior.numPoints();
  ...
  for (size_t ringIndex = 0; ringIndex < polygon.numInteriorRings();
       ++ringIndex) {
    const LineString &hole = polygon.interiorRingN(ringIndex);
    const size_t      numHoleSegs =
        hole.isClosed() ? hole.numPoints() - 1 : hole.numPoints();
              ...
              CoordinateType dimension) -> std::unique_ptr<Polygon>
{
  ...
  for (auto holeIt = face->holes_begin(); holeIt != face->holes_end();
       ++holeIt) {
    auto               holeCcb  = *holeIt;
      ...
      NoValidityCheck /*unused*/) -> std::unique_ptr<Geometry>
{
  ...
  const size_t numIntersections =
      countBoundaryIntersections(polygon, linestring);
  ...
  try {
    ...
    for (auto faceIt = arr.faces_begin(); faceIt != arr.faces_end(); ++faceIt) {
      if (!faceIt->is_unbounded()) {
        if (isFaceInsideOriginalPolygon(faceIt, polygon, segmentStore,
                                        dimension)) {
      ...
      NoValidityCheck /*unused*/) -> std::unique_ptr<Geometry>
{
  ...
  default:
    BOOST_THROW_EXCEPTION(Exception("split: unsupported geometry type " +
