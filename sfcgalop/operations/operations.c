...
bool print_operation_help(const char* name) {
    ...
    printf("Output: %s (%s)\n", 
           ...
           op->result_type == RESULT_TEXT ? "Text" : "Unknown");
    ...
    if (op->param_count > 0 && op->params) {
        ...
        for (int i = 0; i < op->param_count; i++) {
            printf("  %s: %s (default: %.6g)\n", 
                   op->params[i].name,
                   op->params[i].description,
                   op->params[i].default_value);
        }
        printf("\nParameters can be specified by position (comma-separated) or by name (name=value).\n");
