                ...
                const sfcgal_geometry_t *geom_b)
{
  ...
  result.geometry_result =
      sfcgal_geometry_alpha_shapes(geom_a, alpha, allow_holes);
                        ...
                        const sfcgal_geometry_t *geom_b)
{
  ...
  /* Validate nb_components parameter */
  if (nb_components_int < 1 ||
      nb_components_int > 10000) { /* Reasonable limit */
    result.error         = true;
    result.error_message = "nb_components must be between 1 and 10000";
    return result;
  }
  nb_components = (size_t)nb_components_int;

  result.geometry_result =
      sfcgal_geometry_optimal_alpha_shapes(geom_a, allow_holes, nb_components);
                     ...
                     const sfcgal_geometry_t *geom_b)
{
  ...
  if (!parse_int_parameter(op_arg, "relative_alpha", 0, 100,
                           &relative_alpha_int) ||
      !parse_int_parameter(op_arg, "relative_offset", 1, 100,
                           &relative_offset_int)) {
    result.error         = true;
    result.error_message = "Invalid parameter value(s)";
    return result;
  ...
  if (relative_alpha_int < 1 || relative_alpha_int > 10000 ||
      relative_offset_int < 1 || relative_offset_int > 10000) {
    result.error = true;
    result.error_message =
        "relative_alpha and relative_offset must be between 1 and 10000";
    return result;
  }
  ...
  result.geometry_result = sfcgal_geometry_alpha_wrapping_3d(
      geom_a, relative_alpha, relative_offset);
