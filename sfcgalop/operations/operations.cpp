...
const std::vector<Operation> operations = {
    ...
    {"project_medial_axis", "Construction",
     "Project medial axis segments to polygon edges", false,
     "Project the medial axis segments to the polygon boundary to create ridge "
     "lines.\n"
    ...
    {"generate_flat_roof", "Roof Generation",
     ...
     "default=0.0)\n\n"
     "Examples:\n"
     "  sfcgalop -a \"POLYGON((0 0,10 0,10 6,0 6,0 0))\" generate_flat_roof "
     "\"height=2.0\"\n"
     "  sfcgalop -a \"POLYGON((0 0,10 0,10 6,0 6,0 0))\" generate_flat_roof "
     ...
     [](const std::string &params_str, const SFCGAL::Geometry *geom_a,
       ...
       if (building_height == 0.0) {
         ...
         auto building =
             SFCGAL::algorithm::extrude(*polygon, 0, 0, building_height);
         ...
         auto result = std::make_unique<SFCGAL::PolyhedralSurface>(
             building->as<SFCGAL::Solid>().exteriorShell());
    ...
    {"generate_hipped_roof", "Roof Generation",
     ...
     "default=0.0)\n\n"
     "Examples:\n"
     "  sfcgalop -a \"POLYGON((0 0,10 0,10 6,0 6,0 0))\" generate_hipped_roof "
     "\"height=3.0\"\n"
     "  sfcgalop -a \"POLYGON((0 0,10 0,10 6,0 6,0 0))\" generate_hipped_roof "
     ...
     [](const std::string &params_str, const SFCGAL::Geometry *geom_a,
       ...
       if (building_height == 0.0) {
         ...
         return SFCGAL::algorithm::extrudeStraightSkeleton(*polygon,
                                                           roof_height);
       } else {
         // Combine with building
         auto building =
             SFCGAL::algorithm::extrude(*polygon, 0, 0, building_height);
         auto roof =
             SFCGAL::algorithm::extrudeStraightSkeleton(*polygon, roof_height);
    ...
    {"generate_skillion_roof", "Roof Generation",
     ...
     [](const std::string &params_str, const SFCGAL::Geometry *geom_a,
       ...
       if (!polygon) {
         std::cerr
             << "Error: generate_skillion_roof requires a POLYGON input\\n";
         return std::nullopt;
       }
       ...
       if (slope_angle <= 0.0 || slope_angle >= 90.0) {
         std::cerr
             << "Error: slope_angle must be between 0 and 90 degrees, got: "
             << slope_angle << "\\n";
