...
/**
 * @brief Writes an OperationResult to standard output using the requested
 * format and precision.
 *
 * This prints the value contained in `result` to stdout:
        ...
        if constexpr (std::is_same_v<T, std::unique_ptr<SFCGAL::Geometry>>) {
          if (arg) {
            ...
            case OutputFormat::WKT:
              std::cout << arg->asText(precision) << "\n";
              break;
            case OutputFormat::WKB:
              std::cout << arg->asWkb(boost::endian::order::native, true)
                        << "\n";
              break;
          ...
          std::cout << std::setprecision(precision) << arg << "\n";
