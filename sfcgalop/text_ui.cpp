...
/**
 * @brief Compute the display width of a UTF-8 string, ignoring ANSI escape
 * sequences.
 *
 * This function strips ANSI SGR sequences (e.g. "\033[31m") then counts
 * visible characters in the cleaned UTF-8 string, treating each Unicode
 * codepoint (1–4 byte UTF-8 sequence) as width 1. Invalid or continuation
 * bytes are skipped.
 *
 * @param str Input string to compute display width for
 * @return Number of character cells required to render the string
 *         (does not attempt to handle East Asian wide characters or
 *         combining marks specially).
 */
auto
...
/**
 * @brief Removes ANSI Select Graphic Rendition (SGR) sequences from the input
 * string Removes ANSI Select Graphic Rendition (SGR) sequences (e.g.
 * `\033[31m`,
 * `\033[0m`) from the input so the returned string contains no
 * color/formatting escape codes. Note: only SGR sequences ending with 'm' are
 * removed; other terminal control sequences are left intact.
 * @param str Input string that may contain ANSI SGR escape sequences
 * @return Copy of `str` with ANSI SGR sequences removed
 */
auto
strip_ansi(const std::string &str) -> std::string
{
  static const std::regex ansi_regex("\033\\[[0-9;]*m");
