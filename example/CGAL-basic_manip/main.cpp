...
main()
{
  ...
  {
    Segment_2 ab(Point_2(0.0, 0.0), Point_2(1.0, 1.0));
  ...
  std::cout << "--- Polygon_2 ----" << std::endl;
  {
    ...
    // some basic test
    std::cout << "polygon.area()                  : " << polygon.area()
              << std::endl;
    std::cout << "polygon.is_convex()             : " << polygon.is_convex()
              << std::endl;
    std::cout << "polygon.is_clockwise_oriented() : "
              << polygon.is_clockwise_oriented() << std::endl;
    std::cout << "polygon.bbox()                  : " << polygon.bbox()
              << std::endl;
    std::cout << "polygon.is_simple()             : " << polygon.is_simple()
              << std::endl;
  ...
  std::cout << "--- Polygon_with_holes_2 ----" << std::endl;
  {
    ...
    Polygon_with_holes_2 polygon(
        Polygon_2(exteriorRing.begin(), exteriorRing.end()));
  ...
  {
    Segment_3 ab(Point_3(0.0, 0.0, 0.0), Point_3(3.0, 4.0, 5.0));
  ...
  {
    Triangle_3 tri(Point_3(0.0, 0.0, 1.0), Point_3(1.0, 0.0, 1.0),
                   Point_3(0.0, 1.0, 1.0));
  ...
  std::cout << "--- 2D Arrangements ---" << std::endl;
  {
    ...
    std::cout << "autointersects ? "
              << (arr.number_of_vertices() > 4 ? "YES" : "NO") << std::endl;
    ...
    for (it = arr.faces_begin(); it != arr.faces_end(); it++) {
      std::cout << "unbounded: " << it->is_unbounded()
                << " has holes: " << (it->holes_begin() != it->holes_end())
                << std::endl;
    }
  }
  return 0;
