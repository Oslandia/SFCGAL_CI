...
BOOST_AUTO_TEST_CASE(testPointPointCoversPoints)
{
  ...
  BOOST_CHECK_EQUAL(detail::algorithm::coversPoints(pta, ptb), true);
  BOOST_CHECK_EQUAL(detail::algorithm::coversPoints(pta, ptc), false);
  BOOST_CHECK_EQUAL(detail::algorithm::coversPoints3D(pta, ptb), true);
  BOOST_CHECK_EQUAL(detail::algorithm::coversPoints3D(pta, ptc), false);
}
...
{
  {
    ...
    BOOST_CHECK_EQUAL(detail::algorithm::coversPoints(*p1, *p2), true);
    BOOST_CHECK_EQUAL(detail::algorithm::coversPoints3D(*p1, *p2), true);
  }
...
BOOST_AUTO_TEST_CASE(testCollectionCoversPoints)
{
#if 0
    {
        ...
        std::cout << "p1 covers p2 ? " << detail::algorithm::coversPoints3D( *p1, *p2 ) << std::endl;
        std::cout << "p2 covers p1 ? " << detail::algorithm::coversPoints3D( *p2, *p1 ) << std::endl;
    }
