...
public:
    ...
    double area3D(const std::string& wkt) const {
        try {
            ...
            if (!geom) return 0.0;
            if (!SFCGAL::algorithm::isValid(*geom)) {
                std::cerr << "Warning: Invalid geometry for area3D calculation" << std::endl;
                return 0.0;
            }
    ...
    val extrudeDetailed(const std::string& wkt, double height, const val& options) {
        ...
        try {
            ...
            // volume algorithm doesn't exist, calculate manually or use area3D
            result.set("volume", extruded ? CGAL::to_double(SFCGAL::algorithm::area3D(*extruded)) : 0.0);
