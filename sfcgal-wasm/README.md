...
2. **Build dependencies** (GMP, MPFR, Boost, CGAL):
   ...
   **Recommended (No-Patch Method):**
   ```bash
   ./scripts/build-deps-no-patch.sh
   ```
   Builds dependencies without modifying CGAL files. Clean and maintainable.

   **Legacy (Patched Method):**
   ```bash
   ./scripts/build-deps.sh
   ```
   Patches CGAL CMake files for compatibility. Proven and reliable.
...
./scripts/build.sh
```
This script automatically detects which SFCGAL library is available (no-patch or patched) and uses it to build the final WebAssembly module (takes ~20 seconds).

> **Note:** See `cmake/README.md` for detailed comparison of both build methods.

### Quick Build (All-in-One)

**No-Patch Method (Recommended):**
```bash
./scripts/build-all-no-patch.sh
```

**Patched Method (Legacy):**
```bash
# Build deps + patch CGAL
./scripts/build-deps.sh
# Build SFCGAL with patches
./scripts/build-sfcgal-wasm.sh
# Build binding (auto-detects patched lib)
./scripts/build.sh
```

### Build Methods Comparison

| Feature | No-Patch (Recommended) | Patched (Legacy) |
|---------|----------------------|------------------|
| **CGAL Modifications** | ✅ None | ❌ Patches CMake files |
| **Maintainability** | ✅ High | ⚠️ Medium |
| **CGAL Updates** | ✅ Easy | ⚠️ May break |
| **Build Script** | `build-all-no-patch.sh` | `build-deps.sh` + `build-sfcgal-wasm.sh` |
| **Status** | ✅ Validated | ✅ Proven |
| **Library Output** | `build/sfcgal-wasm-no-patch/` | `build/sfcgal-wasm/` |

Both methods produce identical WebAssembly binaries. The `build.sh` script automatically detects and uses whichever library is available.
...
See `examples/LIMITATIONS.md` for detailed information about:

- **Memory constraints** in WebAssembly (~2GB maximum)
- **3D Boolean operations** and when to use 2D vs 3D algorithms
- **Planar geometries** with 3D algorithms (use 2D algorithms for better results)
- **Best practices** for WebAssembly demonstrations

**Quick Tips:**
- Use 2D algorithms (`intersection`, `union`, `difference`) for planar geometries
- Use 3D algorithms (`intersection3D`, `union3D`, `difference3D`) only for truly 3D geometries
- Keep geometries simple in browser (< 100 vertices recommended)
- Always validate geometries with `isValid()` before complex operations

## Troubleshooting

### Build Fails with CMake/Boost Errors

The build scripts automatically patch CGAL CMake files. If this fails:
```bash
rm -rf deps/install/
./scripts/build-deps.sh
```

### "CGAL error: assertion violation! Wrong rounding"

This is handled automatically by `-DCGAL_DISABLE_ROUNDING_MATH_CHECK` flag in the build scripts.

### "Cannot allocate memory" Error

3D operations require significant memory. Simplify geometries or use 2D algorithms for planar geometries.

### Geometries Not Rendering in 3D Viewer

Check browser console for parsing errors. The Three3D renderer logs detailed information about geometry parsing.
