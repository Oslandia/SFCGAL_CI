...
if [ "$CLEAN" = true ]; then
    ...
    [ -d "build/sfcgal-wasm-no-patch" ] && echo "  - build/sfcgal-wasm-no-patch/"
    ...
    rm -rf build/sfcgal-wasm-no-patch 2>/dev/null && log_success "Removed build/sfcgal-wasm-no-patch/"
...
# Step 4: Build dependencies (no patches)
log_step "Step 2/4: Building dependencies (GMP, MPFR, Boost, CGAL)"
log_info "This step takes ~15 minutes"
echo

if [ -f "deps/install/lib/libgmp.a" ] && [ -f "deps/install/lib/cmake/CGAL/CGALConfig.cmake" ]; then
    log_info "Dependencies already built"

    # Check if patched
    if [ -f "deps/install/lib/cmake/CGAL/CGAL_SetupBoost.cmake.bak" ]; then
        log_error "Existing dependencies are PATCHED"
        log_info "Cleaning and rebuilding without patches..."
        rm -rf deps/install/lib/cmake/CGAL
        ./scripts/build-deps-no-patch.sh 2>&1 | tee -a "$LOG_FILE"
    else
        log_success "Using existing no-patch dependencies"
    fi
else
    ./scripts/build-deps-no-patch.sh 2>&1 | tee -a "$LOG_FILE"
fi
...
if [ -f "build/sfcgal-wasm-no-patch/src/libSFCGAL.a" ]; then
    log_info "SFCGAL library already built"

    # Verify it's actually a WASM library
    if file build/sfcgal-wasm-no-patch/src/libSFCGAL.a | grep -q "LLVM"; then
        log_success "Using existing SFCGAL WebAssembly library"
    else
        log_error "Existing library is NOT WebAssembly, rebuilding..."
        rm -rf build/sfcgal-wasm-no-patch
        source build/emsdk/emsdk_env.sh
        ./scripts/build-sfcgal-wasm-no-patch.sh 2>&1 | tee -a "$LOG_FILE"
    fi
else
    source build/emsdk/emsdk_env.sh
    ./scripts/build-sfcgal-wasm-no-patch.sh 2>&1 | tee -a "$LOG_FILE"
...
log_success "All components built successfully using NO-PATCH method"
echo
